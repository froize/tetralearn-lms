...
class ChatController extends AppAdminController
{
    ...
    public function actionIndex()
    {
        ...
        if(($dialog_id['student_id']!= $user->id) && ($dialog_id['tutor_id'] != $user->id)) {
            $dialog_id = 0;
        }
        else {
            $dialog_id = $dialog_id['id'];
        }
        $dialogs = Dialog::find()->asArray()->where(['tutor_id'=> $user->id])->all();
        $dialogs_messages = array();
        $k =0 ;
        foreach ($dialogs as $dialog) {
            $dialogs_messages[$k] = Chat::getMessages($this->module->numberLastMessages, $dialog['id']);
            $k++;
        }

        if($dialog_id != 0) {
            $messages = Chat::getMessages($this->module->numberLastMessages, $dialog_id);
